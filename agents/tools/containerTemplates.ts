// src/tools/containerTemplates.ts
import { z } from "zod";
import * as k8s from "@kubernetes/client-node";

// 1) Allowed built-in templates
...
// 2) Schema for a user-supplied “custom” container
...
export type CustomContainerConfigType = z.infer<
  typeof CustomContainerConfig
>;

// 3) The built-in container specs
...
> = {
  ubuntu: {
    ...
    livenessProbe: {
      exec: { command: ["cat", "/proc/1/status"] },
  ...
  custom: {
    // This will get overwritten by your CustomContainerConfig at runtime
    name:  "main",
    image: "busybox:latest",
    command: ["sh"],
    args:    ["-c", "sleep infinity"],
